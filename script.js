...
for (let i = 0; i < 20; i++) {
  ...
  icon.classList.add(
    "fa-solid",
    iconosPosibles[Math.floor(Math.random() * iconosPosibles.length)],
    "floating-element",
  );
...
window.verificarYMostrarAnuncio = async () => {
  ...
  if (sessionStorage.getItem("anuncioVisto")) {
    console.log(
      "2. Cancelado: El navegador dice que ya viste el anuncio en esta sesion.",
    );
    return;
  }
    ...
    if (snap.exists()) {
      ...
      if (data.activo && data.imagen) {
        ...
        setTimeout(() => {
          ...
          document.getElementById("announcement-overlay").style.display =
            "flex";
        ...
        console.log(
          "4. Cancelado: El anuncio esta DESACTIVADO o no tiene puesto un link de imagen.",
        );
      }
    } else {
      console.log(
        "3. Cancelado: Todavia no se guardo ningun anuncio en la base de datos.",
      );
    }
  } catch (e) {
...
document.getElementById("btn-guest-login").onclick = () => {
  ...
  document.getElementById("contact-user-info").innerText =
    "Modo Invitado (Sin envio)";
...
onAuthStateChanged(auth, async (user) => {
  setTimeout(async () => {
    if (user) {
      ...
      if (user.photoURL) {
        document.getElementById("btnProfile").innerHTML =
      ...
      try {
        ...
        if (userDoc.exists()) {
          ...
          if (!userData.visitedGames)
            await updateDoc(doc(db, "usuarios", user.uid), { visitedGames: 0 });
        } else {
...
window.toggleMenu = (btn, id) => {
  ...
  document
    .querySelectorAll(".profile-dropdown, .notif-dropdown")
    .forEach((d) => {
      if (d.id !== id) d.style.display = "none";
    });
  document
    .querySelectorAll("#menuClosed button")
    .forEach((b) => b.classList.remove("active-menu"));
...
btnProfile.onclick = (e) => {
  e.stopPropagation();
  window.toggleMenu(btnProfile, "profileDropdown");
};
btnNotif.onclick = (e) => {
  e.stopPropagation();
  window.toggleMenu(btnNotif, "notifDropdown");
...
// Variable para saber qué juego estamos seleccionando con las flechas
let currentFocus = -1;

// 1. EVENTO DE TECLADO (Flechitas y Enter)
searchInput.addEventListener("keydown", function (e) {
  ...
  if (e.key === "ArrowDown") {
    ...
    e.preventDefault();
    if (currentFocus > -1) {
      // Si usaste las flechas, entra al juego seleccionado
      if (x && x[currentFocus]) x[currentFocus].click();
    } else {
      // Si solo tocaste Enter, cierra la lista
      autocompleteList.style.display = "none";
      this.blur();
    }
  }
...
function addActive(x) {
  ...
  searchInput.value = x[currentFocus].innerText;
}
...
// 2. EVENTO AL ESCRIBIR (Filtro, mostrar cruz y limpiar géneros)
searchInput.addEventListener("input", function () {
  const val = normalizarTexto(this.value);

  // Lógica para la cruz y los géneros
  if (this.value.length > 0) {
    document.getElementById("btnClearSearch").style.display = "block";
    document
      .querySelectorAll(".adv-genre-chk")
      .forEach((c) => (c.checked = false));
  } else {
    document.getElementById("btnClearSearch").style.display = "none";
  }

  currentFocus = -1; // Reinicia las flechas al escribir
  ...
  const matches = juegos
    .filter((j) => normalizarTexto(j.titulo).includes(val))
    .slice(0, 5);
...
// 3. EVENTO DE LA CRUZ (Para borrar el texto rápido)
...
window.aplicarFiltrosGlobales = () => {
  ...
  const selectedGenres = Array.from(
    document.querySelectorAll(".adv-genre-chk:checked"),
  ).map((c) => c.value.toLowerCase());
  const selectedReqs = Array.from(
    document.querySelectorAll(".adv-req-chk:checked"),
  ).map((c) => c.value.toLowerCase());
  const sortMode =
    document.querySelector('input[name="adv_sort"]:checked')?.value ||
    "date_release_new";

  let juegosBase = isViewingFavs
    ? juegos.filter((j) => favoritos.includes(j.titulo))
    : juegos;
  ...
  if (sortMode === "rating") {
    juegosFiltrados.sort(
      (a, b) => parseFloat(getAvgRating(b)) - parseFloat(getAvgRating(a)),
    );
    ...
    juegosFiltrados.sort(
      (a, b) => new Date(b.fechaSalida || 0) - new Date(a.fechaSalida || 0),
    );
  } else if (sortMode === "date_release_old") {
    juegosFiltrados.sort(
      (a, b) => new Date(a.fechaSalida || 0) - new Date(b.fechaSalida || 0),
    );
...
function renderizarChecksGeneros(mode) {
  ...
  const cont = document.getElementById(
    mode === "new" ? "add-genre-grid" : "edit-genre-grid",
  );
  if (!cont) return;
  cont.innerHTML = "";
  listaGeneros.forEach((g) => {
    const extraStyle =
      g === "Multijugador" || g === "Pantalla Dividida"
        ? 'style="accent-color:#00ff00;"'
        : "";
...
window.agregarGeneroNuevo = async () => {
  ...
  if (!val) return;
  if (listaGeneros.includes(val)) {
    showToast("El genero ya existe", "error");
    return;
  }
...
async function cargarJuegos() {
  ...
  juegos.sort(
    (a, b) =>
      new Date(b.fechaSalida || b.fecha) - new Date(a.fechaSalida || a.fecha),
  );
  ...
  if (gameId) {
    ...
    if (targetGame) {
      history.replaceState(
        { view: "game", gameId: targetGame.id },
        "",
        "?game=" + targetGame.id,
      );
...
function renderizarJuegos() {
  ...
  if (pagina.length === 0) {
    gameList.innerHTML =
      "<h3 style='grid-column: 1/-1; text-align:center; color:#666;'>No se encontraron juegos.</h3>";
    return;
  }
  ...
  pagina.forEach((juego) => {
    ...
    card.onclick = (e) => {
      if (!e.target.closest(".heart")) window.abrirDescargas(juego);
    };
...
window.accionUsuario = async (accion) => {
  ...
  if (accion === "banear") {
    const targetSnap = await getDoc(
      doc(db, "usuarios", usuarioSeleccionadoUid),
    );
    if (
      targetSnap.exists() &&
      (targetSnap.data().rol === "admin" || targetSnap.data().rol === "dios")
    ) {
      showToast(
        "PROTECCION ADMIN: No puedes banear a un Administrador.",
        "error",
      );
      return;
    }
    ...
    if (confirm("Desbanear usuario y borrar historial (Desvanecer)?")) {
      const comentariosRef = query(
        ...
        where("uid", "==", usuarioSeleccionadoUid),
      );
      const comentariosSnap = await getDocs(comentariosRef);
      const deletePromises = comentariosSnap.docs.map((doc) =>
        deleteDoc(doc.ref),
      );
...
window.cargarNotificaciones = () => {
  ...
  lista.innerHTML = "";
  const ultimos = [...juegos]
    .sort((a, b) => new Date(b.fecha) - new Date(a.fecha))
    .slice(0, 5);
  if (ultimos.length === 0)
    lista.innerHTML =
      "<div style='padding:10px; color:#aaa'>No hay novedades.</div>";
  ultimos.forEach((j) => {
    const div = document.createElement("div");
...
window.preVote = (val) => {
  ...
  if (usuarioBaneado)
    return showToast("Estas baneado, no puedes votar.", "error");
...
window.submitVote = async (valorVoto) => {
  if (!usuarioActual) {
    showToast("Inicia sesion para votar.", "error");
    return;
  }
...
window.enviarComentario = async () => {
  if (esInvitado)
    return showToast("No puedes comentar en Modo invitado.", "error");
  if (!usuarioActual) {
    showToast("Inicia sesion para comentar.", "error");
    return;
  }
...
async function cargarComentarios(gameId, highlightId = null) {
  ...
  try {
    ...
    if (snap.empty) {
      list.innerHTML =
        "<p style='color:#888; text-align:center;'>Se el primero en comentar.</p>";
      ...
      if (replies.length > 0) {
        ...
        toggleBtn.onclick = () => {
          ...
          toggleBtn.innerHTML = isHidden
            ...
            : `<span>---- Ver ${replies.length} respuestas ----</span>`;
        };
    ...
    console.error(e);
    list.innerHTML = "<p>Error cargando comentarios.</p>";
  }
}
...
function renderCommentItem(container, c, gameId, isReply, isHighlight) {
  ...
  const avatarUrl =
    c.photo ||
    "https://ui-avatars.com/api/?name=" + c.user + "&background=random";
  const pinIcon = c.anclado
  ...
  if (c.role === "dios") roleBadge = `<span class="role-tag dios">DIOS</span>`;
    ...
    roleBadge = `<span class="role-tag admin">ADMIN</span>`;
...
window.anclarComentario = async (gameId, commentId, estado) => {
  try {
    await updateDoc(doc(db, "juegos", gameId, "comentarios", commentId), {
      anclado: estado,
    });
...
window.enviarApelacion = async () => {
  ...
  try {
    ...
    showToast("Error al enviar apelacion.", "error");
  }
};
...
document.getElementById("req-game-input").addEventListener("input", () => {
  const val = document
    .getElementById("req-game-input")
    .value.toLowerCase()
    .trim();
  if (val.length >= 3) {
    const exists = peticionesCache.some((p) =>
      normalizarTexto(p.game).includes(normalizarTexto(val)),
    );
    if (exists) {
      document.getElementById("req-status-message").innerText =
        "Este juego ya esta en peticiones.";
...
window.abrirMuroPeticiones = async () => {
  ...
  if (reqs.length === 0) {
    list.innerHTML =
      "<p style='color:#888; text-align:center;'>No hay peticiones. Se el primero en pedir un juego.</p>";
  }
...
window.marcarPeticionAgregada = async (reqId) => {
  if (!esAdmin && !esDios) return;
  if (
    confirm("Marcar esta peticion como agregada? Se eliminara de la lista.")
  ) {
    await deleteDoc(doc(db, "peticiones", reqId));
  ...
  try {
    ...
    if (snap.exists()) {
      ...
      document.getElementById("admin-announcement-image").value =
        data.imagen || "";
      document.getElementById("admin-announcement-link").value =
        data.link || "";
    }
  } catch (e) {
...
document
  .getElementById("manual-ban-input")
  .addEventListener("input", function () {
    ...
    const matches = allUsersCache
      .filter((email) => email.toLowerCase().includes(val))
      .slice(0, 5);
    if (matches.length > 0) {
...
window.desbanearDesdePanel = async (uid) => {
  if (
    confirm("Desbanear usuario? Se borrara todo su historial (Nacer de nuevo).")
  ) {
    const comentariosRef = query(
      collectionGroup(db, "comentarios"),
      where("uid", "==", uid),
...
async function cargarListaAdmins() {
  ...
  const qAdmin = query(
    ...
    where("rol", "in", ["admin", "dios"]),
  );
  const snap = await getDocs(qAdmin);

  cont.innerHTML = "";
  if (snap.empty) {
    cont.innerHTML = "No hay admins.";
    return;
  }
...
document
  .getElementById("input-admin-email")
  .addEventListener("input", function () {
    const val = this.value.toLowerCase();
...
window.quitarRangoAdmin = async (uid) => {
  if (
    confirm(
      "Eliminar rango de administrador a este usuario? Volvera a ser usuario normal.",
    )
  ) {
...
window.borrarJuegoPermanentemente = async (gameId, title) => {
  if (
    confirm(
      `ADVERTENCIA: Eliminar PERMANENTEMENTE el juego "${title}"? Esta accion no se puede deshacer.`,
    )
  ) {
...
window.abrirDescargas = function (juego, pushState = true) {
  if (pushState) {
    history.pushState(
      { view: "game", gameId: juego.id },
      "",
      "?game=" + juego.id,
    );
  }

  document.getElementById("game-details-bg-blur").style.backgroundImage =
    `url('${juego.imagen}')`;
  ...
  const reqText = juego.requisito
    ? ` | ${juego.requisito.charAt(0).toUpperCase() + juego.requisito.slice(1)}`
    : "";
  ...
  const enlacesRaw =
    juego.enlaces ||
    (juego.link
      ? [{ servidor: "Descarga Directa", url: juego.link, nota: "Link" }]
      : []);

  if (enlacesRaw.length === 0) {
    containerLinks.innerHTML =
      "<p style='color:#aaa'>No hay enlaces disponibles.</p>";
  } else {
    const groups = {};
    ...
    for (const [server, links] of Object.entries(groups)) {
      ...
      else if (srvLower.includes("drive") || srvLower.includes("google"))
...
window.pegarEnInput = async (btn) => {
  try {
    ...
    showToast("No se pudo pegar. Permiso denegado.", "error");
  }
};
...
const getLinks = (id) =>
  Array.from(document.querySelectorAll(`#${id} .link-row`))
    .map((r) => {
...
const getGens = (cls) =>
  Array.from(document.querySelectorAll(`.${cls}:checked`)).map((c) => c.value);

document
  .getElementById("form-add-game")
...
window.abrirEditar = (id) => {
  ...
  document
    .querySelectorAll(".chk-genre-edit")
    .forEach((c) => (c.checked = (j.generos || []).includes(c.value)));
  const cont = document.getElementById("edit-links-container");
  cont.innerHTML = "";
  const links =
    j.enlaces ||
    (j.link ? [{ servidor: "Mediafire", url: j.link, nota: "Descarga" }] : []);
  links.forEach((l) =>
    window.agregarInputLink(
...
window.toggleFav = async (t, btn) => {
  ...
  const todosLosCorazones = document.querySelectorAll(
    `[onclick*="toggleFav('${safeTitle}'"] i`,
  );
...
window.toggleMostrarFavoritos = (btn) => {
  ...
  document.querySelector(
    'input[name="adv_sort"][value="date_release_new"]',
  ).checked = true;
...
function getDragAfterElement(container, y) {
  const draggableElements = [
    ...container.querySelectorAll(".link-row:not(.dragging)"),
  ];
